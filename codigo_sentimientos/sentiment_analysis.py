...
num_recs=0
ftrain=open(os.path.join("/home/jorge/Documentos/Datos_complejos/Proyecto_final/DATA_DIR", "training.txt"), 'r')
for line in ftrain:
...
#Usamos un estimado de la longitud maxima de frecuencia de las palabras y del largo de las sentencias
MAX_FEATURES=2000
...
#Probamos el modelo con los datos de imdb
maxlen=0
word_freqs=collections.Counter()
num_recs=0
ftrain=open(os.path.join("/home/jorge/Documentos/Datos_complejos/Proyecto_final/DATA_DIR", "imdb_labelled.txt"), 'r')
for line in ftrain:
...
#Preprocesamos de la misma forma que con los datos anteriores
word2index = {x[0]: i+2 for i,x in enumerate(word_freqs.most_common(MAX_FEATURES))}
word2index["PAD"]=0
word2index["UNK"]=1
index2word={v:k for k, v in word2index.items()}
...
score, acc = model.evaluate(X, y, batch_size=BATCH_SIZE)    #Guardamos nuestros valores de precision y error obtenidos
print("\n\n")
print("Test score: %.3f, Precisión: %.3f" % (score, acc))
...
#Preprocesamos de la misma forma que con los primeros datos
word2index = {x[0]: i+2 for i,x in enumerate(word_freqs.most_common(MAX_FEATURES))}
word2index["PAD"]=0
word2index["UNK"]=1
index2word={v:k for k, v in word2index.items()}
...
word_freqs=collections.Counter()
num_recs=0
ftrain=open(os.path.join("/home/jorge/Documentos/Datos_complejos/Proyecto_final/DATA_DIR", "yelp_labelled.txt"), 'r')
for line in ftrain:
	sentence,label = line.strip().split("\t")
	words=nltk.word_tokenize(sentence.lower())
	if len(words)>maxlen:
